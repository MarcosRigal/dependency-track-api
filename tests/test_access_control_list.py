"""Tests for the access control list module."""
...
def test_add_acl_mapping_success(dependency_track_api):
    ...
    dependency_track_api.add_acl_mapping = MagicMock(
        return_value={
            "team": "d15a83fd-da70-9387-a698-6e9bc1ec1591",
            "project": "3cc0deca-3f9f-516c-30e0-ce14e7cdcdfb",
        }
...
def test_add_acl_mapping_project_not_found(dependency_track_api):
    ...
    dependency_track_api.add_acl_mapping = MagicMock(
        side_effect=DependencyTrackApiError(
            "The UUID of the team or project could not be found",
            MockResponse(
...
def test_add_acl_mapping_mapping_already_exists(dependency_track_api):
    ...
    dependency_track_api.add_acl_mapping = MagicMock(
        side_effect=DependencyTrackApiError(
            "A mapping with the same team and project already exists",
            MockResponse(
                {"message": "A mapping with the same team and project already exists"},
                status_code=409,
            ),
        )
    )
...
def test_delete_acl_mapping_success(dependency_track_api):
    ...
    dependency_track_api.delete_acl_mapping = MagicMock(return_value=None)
    dependency_track_api.delete_acl_mapping(
...
def test_delete_acl_mapping_unauthorized(dependency_track_api):
    ...
    dependency_track_api.delete_acl_mapping = MagicMock(
        side_effect=DependencyTrackApiError(
            "Unauthorized", MockResponse({"message": "Unauthorized"}, status_code=401)
        )
    )
...
def test_delete_acl_mapping_unexpected_status(dependency_track_api):
    ...
    dependency_track_api.delete_acl_mapping = MagicMock(
        side_effect=DependencyTrackApiError(
            "Unexpected status", MockResponse({"message": "Unexpected status"}, status_code=500)
        )
    )
    with pytest.raises(DependencyTrackApiError):
        dependency_track_api.delete_acl_mapping(
...
def test_retrieve_projects_for_team_success(dependency_track_api):
    ...
    dependency_track_api.retrieve_projects_for_team = MagicMock(
        return_value=["project_uuid_1", "project_uuid_2"]
    )
...
def test_retrieve_projects_for_team_team_or_project_not_found(dependency_track_api):
    ...
    dependency_track_api.retrieve_projects_for_team = MagicMock(
        side_effect=DependencyTrackApiError(
            "The UUID of the team, project, or team could not be found",
            MockResponse(
